...
const AppliedJobs = () => {
    ...
    const [aspirantAppliedJobs, setAspirantApplied] = useState([])
    ...
    useEffect(() => {
        ...
        try {
            const fetchResponse = await fetch(url, settings);
            const response = await fetchResponse.json();
            setAspirantApplied(response.applications);
            
            
            // jobs.objArrayOfJobs = jobs.objArrayOfJobs.filter((e) => {
            //     if(aspirantAppliedJobs.contains(e._id)){
            //         return e
            //     }
            // })
            } catch (e) {
    ...
    return(
        ...
        {   
            ...
            jobs.objArrayOfJobs && jobs.objArrayOfJobs.length > 0 ?

            
                    jobs.objArrayOfJobs.filter((e) => {
                        for(let i = 0;i<aspirantAppliedJobs.length;i++){
                            if(aspirantAppliedJobs[i].jobId === e._id){
                                return e
                            }
                        }
                    }).map((job) => (
